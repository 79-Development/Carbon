...
client.on(Events.ChannelUpdate, async (channel) => {
    const auditLogs = await channel.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.ChannelOverwriteCreate }).catch((_) => { });
    ...
    await client.db.get(`${channel.guild.id}_wl`).then(async (data) => {
        await client.db.get(`${channel.guild.id}_owner`).then(async (data2) => {
            ...
            const chann = await client.db.get(`${channel.guild.id}_logschannel`)
