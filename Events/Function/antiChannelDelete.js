...
client.on("channelDelete", async (channel) => {
    const auditLogs = await channel.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.ChannelDelete }).catch((_) => { });
    ...
    await client.db.get(`${channel.guild.id}_wl`).then(async (data) => {
        await client.db.get(`${channel.guild.id}_owner`).then(async (data2) => {
            ...
            await channel.clone().then(ch => ch.setPosition(channel.position)).catch((_) => { });
        });
    });
});