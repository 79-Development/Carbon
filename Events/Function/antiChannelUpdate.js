...
client.on("channelUpdate", async (o, n) => {
    const auditLogs = await n.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.ChannelUpdate }).catch((_) => { });
    ...
    await client.db.get(`${n.guild.id}_wl`).then(async (data) => {
        await client.db.get(`${n.guild.id}_owner`).then(async (data2) => {
            ...
            if (oldName !== newName) {
                await n.edit({ name: oldName }).catch(() => {});
            }
            ...
            if (n.type === 0) { 
                ...
                if (oldTopic !== newTopic) {
                    await n.setTopic(oldTopic).catch(() => {});
                }
            }
            if (n.type === 2) { 
                if (o.bitrate !== n.bitrate) {
                    n.edit({ bitrate: o.bitrate }).catch(() => {});
                }
            }
        });
    });
